...
export default defineConfig(({ mode }) => {
  ...
  // - API PHP:      /AP_Restaurante/backend/api/*.php
  ...
  // Ej: GET /api/empleados.php  ->  http://localhost/AP_Restaurante/backend/api/empleados.php
  const backendOrigin = env.VITE_BACKEND_ORIGIN || 'http://localhost'
  const backendPath = env.VITE_BACKEND_PATH || '/AP_Restaurante/backend/api'
  ...
  return {
    ...
    build: {
      // Deja el build dentro de /public/app (carpeta pública en la raíz del proyecto) para que Apache lo sirva
      outDir: '../../public/app',
      emptyOutDir: true,
    },
  }
