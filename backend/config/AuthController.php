...
class AuthController {
    ...
    private function handleLogin($input) {
        ...
        // Datos del colaborador para la respuesta (sin contraseña)
        // Por seguridad, solo mostrar parcialmente RFC y CURP (ya no están cifrados)
        $rfc = $colaborador['rfc'] ?? '';
        ...
        $colaboradorData = [
            ...
            'rfc' => $rfc ? (substr($rfc, 0, 4) . '****' . substr($rfc, -3)) : '', // Solo mostrar parcialmente
            'curp' => $curp ? (substr($curp, 0, 4) . '****' . substr($curp, -4)) : '', // Solo mostrar parcialmente
    ...
    private function generarToken($idColaborador, $correo) {
        // Token simple: base64(id_colaborador:correo:timestamp)
